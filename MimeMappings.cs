...
{
    /// <summary>
    /// A static class used for mapping common file extensions to MimeTypes
    /// </summary>
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        /// <returns>The first matching extension for the mime type, or String.Empty if its not found (for concat)</returns>
        public static string GetFileExtension(string mimeType)
        {
            if (ReverseMimeMapping is null)
            {
                lock (ReverseMappingLock)
                {
                    ReverseMimeMapping = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase);

                    foreach (KeyValuePair<string, string> kvp in _mappings)
                    {
                        if (!ReverseMimeMapping.ContainsKey(kvp.Value))
                        {
                            ReverseMimeMapping.Add(kvp.Value, kvp.Key);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Returns the top level mime type for a given file name (based on extension)
        /// </summary>
        /// <param name="FileName">The file name to check the mime type of</param>
        /// <returns>The top level mime type for a given file name (based on extension)</returns>
